...
#从原始数据集users.csv 和 servers.csv 中挑选指定数量的用户和服务器，并将挑选得到的用户以及服务器放在 selected_users.csv 和 selected_servers.csv 中，并返回选中的用户，服务器，服务器的覆盖半径，以及cov_initial矩阵
...
def reallocate_failed_users(U, S, L_j, L_i_j, c_i_j, c_i_cloud, x_initial_i_j, x_initial_i_cloud, cov_initial, r, omega, d):
    ...
    #计算真正的初分配目标函数值
    if successful_users:
        F_u_initial = np.sum(x_initial_i_j[successful_users]) / len(successful_users)  # 初次成功用户的分配率
